...
const createEventInCalendar = async (task) => {
  ...
  if (
    ...
    !recurrenceRuleValue.startsWith("custom:")
  ) {
    recurrence = {
  ...
  const eventDetails = {
    ...
    notes:
      task.description || "Utworzone z aplikacji do zarządzania zadaniami.",
    timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone,
    alarms: task.isAllDay
      ? []
      : [{ relativeOffset: -10, method: Calendar.AlarmMethod.DEFAULT }],
    recurrence,
  };
...
const TimePicker = ({ time, setTime, disabled }) => {
  ...
  return (
    ...
    >
      <Text
        style={[styles.timeInputText, disabled && styles.timeInputTextDisabled]}
      >
        {time}
      </Text>
      {showPicker && (
...
export default function AddTaskScreen() {
  ...
  const handleSetRecurrence = (ruleValue) => {
    ...
    setShowCustomRecurrencePicker(false);
  };
  ...
  const handleDateChange = (event, selectedDate) => {
    ...
    if (selectedDate) {
      ...
      const currentStartDate =
        datePickerTarget === "start" ? newDateString : startDate;
      const currentEndDate =
        datePickerTarget === "end" ? newDateString : endDate;
  ...
  const toggleDatePicker = (target) => {
    if (datePickerTarget === target && showDatePicker) {
    ...
    >
      <ScrollView style={styles.scrollContent}>
        ...
        <View style={styles.formCard}>
          ...
          {showDatePicker &&
            ...
            Platform.OS !== "web" && (
              <View style={styles.datePickerContainer}>
                <DateTimePicker
                  value={
                    datePickerTarget === "start" ? startDateObj : endDateObj
                  }
                  mode="date"
                  display={Platform.OS === "ios" ? "spinner" : "default"}
          ...
          <TimePicker time={endTime} setTime={setEndTime} disabled={isAllDay} />
        </View>
        ...
        <View style={styles.formCard}>
          ...
          {showCustomRecurrencePicker && Platform.OS !== "web" && (
            ...
            >
              ...
              >
                Wybierz datę zakończenia powtarzania (własne):
              </Text>
              <View style={styles.datePickerContainer}>
                <DateTimePicker
              ...
              <View style={styles.tagsContainer}>
                {allTags
                  ...
                  .map((tag) => (
                    ...
                    >
                      <Text style={[styles.tagText, styles.suggestedTagText]}>
                        {tag}
                      </Text>
                    </TouchableOpacity>
                  ))}
