        ...
        <View style={timePickerStyles.container}>
            ...
            >
                ...
                <View style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', flex: 1}}>
                    <Text style={[timePickerStyles.text, disabled && { color: '#8E8E93' }]}>{time}</Text>
...
  
  // Pobieranie ID zadania z parametrów URL
  const { id: taskId } = useLocalSearchParams();
  
  // Znajdowanie zadania do edycji
  const taskToEdit = tasks.find(t => t.id === taskId);

  // --- INICJALIZACJA STANU ---
  const [name, setName] = useState(taskToEdit?.name || "");
  const [selectedTags, setSelectedTags] = useState(taskToEdit?.hashtags || []);
...
  const [selectedPriority, setSelectedPriority] = useState(taskToEdit?.priority || PRIORITY_OPTIONS[1].value); 
  
  // Daty i Czasy Rozpoczęcia/Zakończenia
  const [startDate, setStartDate] = useState(taskToEdit?.startDate || todayDateString);
  const [startTime, setStartTime] = useState(taskToEdit?.startTime || '09:00');
  const [endDate, setEndDate] = useState(taskToEdit?.deadline || todayDateString); // Używamy 'deadline' jako endDate
  const [endTime, setEndTime] = useState(taskToEdit?.endTime || '10:00');
  
  // NOWE STANY DLA OPCJI KALENDARZA
...
  const [reminderTime, setReminderTime] = useState(taskToEdit?.reminderTime || 'Godzina wydarzenia');
  
  // Stan dla UI
...
      setNewTagInput("");
    } else if (trimmedTag && tags.includes(trimmedTag) && !selectedTags.includes(trimmedTag)) {
        setSelectedTags((prevTags) => [...prevTags, trimmedTag]);
        setNewTagInput("");
    } else if (trimmedTag) {
        Alert.alert("Tag już istnieje", `Tag "${trimmedTag}" jest już aktywny lub wybrany.`);
    }
  };
...
    // Przekazanie wszystkich zaktualizowanych pól
    editTask(taskId, {
      name: name.trim(),
      deadline: endDate,
...
  if (!taskToEdit) {
      return <View style={styles.loadingContainer}><Text>Ładowanie...</Text></View>;
  }
...
        </View>
        ...
        <View style={styles.section}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.iconContainer}>
                {TASK_ICONS.flatMap(category => category.icons).map((icon, index) => (
                    <TouchableOpacity
                        key={`${icon}-${index}`}
                        style={[
                            styles.iconButton,
...
            
            {/* POWIADOMIENIE */}
            <TouchableOpacity style={[styles.calendarOptionRow, { borderBottomWidth: 0 }]} onPress={() => Alert.alert('Powiadomienie', 'Wybór czasu powiadomienia')}>
...
            {Platform.OS === 'web' ? (
                <TextInput
                    style={[styles.input, styles.dateInputWeb, isAllDay && { flex: 1 }]}
                    value={startDate}
...
                />
            ) : (
                <TouchableOpacity style={[styles.dateInput, isAllDay && { flex: 1 }]} onPress={() => showDatepicker('start')}>
                    <Text style={styles.dateText}>{formatFullDate(startDate)}</Text><Text style={styles.dateIcon}>📅</Text>
                </TouchableOpacity>
            )}
            
...
                <>
                    <View style={{ width: 10 }} /> 
                    <TimePicker label="Czas" time={startTime} setTime={setStartTime} disabled={isAllDay} />
                </>
            )}
          </View>
        </View>
...
                />
            ) : (
                <TouchableOpacity style={[styles.dateInput, isAllDay && { flex: 1 }]} onPress={() => showDatepicker('end')}>
                    <Text style={styles.dateText}>{formatFullDate(endDate)}</Text><Text style={styles.dateIcon}>📅</Text>
                </TouchableOpacity>
            )}
            {/* ZMIANA: WARUNKOWE RENDEROWANIE OKIENKA CZASU */}
            {!isAllDay && (
                <>
                    <View style={{ width: 10 }} />
                    <TimePicker label="Czas" time={endTime} setTime={setEndTime} disabled={isAllDay} />
                </>
            )}
          </View>
        </View>
...
    flexDirection: 'row',
    justifyContent: 'space-between',
    // Usunięto gap, ponieważ jest dodawany warunkowo
  },
  dateInput: {
    flex: 1,
    flexDirection: "row",
...
    borderRadius: 10,
  },
  dateInputWeb: { // NOWY STYL DLA INPUTU DATY NA WEB
    flex: 1,
    backgroundColor: "#fff",
    borderWidth: 1,
    borderColor: "#e0e0e0",
...
    textAlign: 'center',
  },
  timeInputDisabled: { // STYL DLA ZABLOKOWANEGO INPUTU CZASU
